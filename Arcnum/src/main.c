...
void CreateEntities(Application app) {
  for (size_t i = 0; i < 10000; i++) {
    ...
    data->age = 20.0f;
    data->id = 1234567890;
    AddComponent(app->ecs, e,
                 CreateComponent(SPARK_TRANSFORM_COMPONENT, "Transform", data,
                                 SparkFree));
  }
}
...
i32 main() {
  Application app = CreateApplication(
      CreateWindow(CreateWindowData("Hello", 1080, 1080, SPARK_FALSE)), 8);
  ...
  AddStartFunctionApplication(app, ResourceCreater,
  ...
  AddStartFunctionApplication(app, TestDeserialization,
                              SPARK_UNBLOCKED_PARRALLELISM);
  AddStartFunctionApplication(app, CreateEntities,
                              SPARK_UNBLOCKED_PARRALLELISM);
