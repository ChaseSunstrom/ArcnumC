...
void_t StartFunction(Application app) {
	for (size_t i = 0; i < 1000; i++) {
		...
		Component component = CreateComponent(SPARK_TRANSFORM_COMPONENT, "Transform",
			"TRANSFORM_COMPONENT", SPARK_NULL);
		AddComponent(app->ecs, entity, component);
		SPARK_LOG_DEBUG(
			"Created entity: %d, Added component: %s", i,
			GetComponent(app->ecs, entity, SPARK_TRANSFORM_COMPONENT, "Transform")->data);
	}
...
i32 main() {
	...
	AddQueryEventFunctionApplication(app, SPARK_EVENT_MOUSE_BUTTON_PRESSED | SPARK_EVENT_MOUSE_BUTTON_RELEASED, SPARK_TRANSFORM_COMPONENT, QueryEventFunction);
