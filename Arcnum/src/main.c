...
void_t StartFunction(Application app) {
  for (size_t i = 0; i < 1000; i++) {
    ...
    Component component = CreateComponent("Transform", "Transform",
                                          "TRANSFORM_COMPONENT", SPARK_NULL);
    AddComponent(app->ecs, entity, component);
    SPARK_LOG_DEBUG(
        "Created entity: %d, Added component: %s", i,
        GetComponent(app->ecs, entity, "Transform", "Transform")->data);
  }
...
i32 main() {
  ...
  AddQueryFunctionApplication(app, "Transform", QueryFunction);
